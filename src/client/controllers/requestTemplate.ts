...
export const updateStatusTemplateRequest = (req: Request, res: Response) => {
    ...
    ChangeTemplateRequest.updateOne({ _id: req.body.id }, body)
        .then((Users) => {
            Mailer.sendMail(
                emailList,
                `Your template request has been ${req.body.status === "pending" ? "Updated" : req.body.status}.`,
                `Template request ${req.body.status === "pending" ? "updated" : "status updated"}`,
                null
            )
                .then((res) => console.log("Email triggered"))
                .catch((err) => console.log("Email not triggered"));
...
export const deleteTemplateRequest = (req: Request, res: Response) => {
    ChangeTemplateRequest.deleteOne({ _id: req.params.id }).then((response: any) => {
        res.json(response);
    });
};

export const getWorkbookTemplateRequestById = (req: Request, res: Response) => {
    ChangeInputWorkbookTemplateRequest.findById({ _id: req.params.id })
